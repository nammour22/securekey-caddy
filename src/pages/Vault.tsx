  ...
  return <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <div className="ml-64 p-6">
        <div className="max-w-2xl mx-auto bg-gray-100">
          <Link to="/" className="inline-flex items-center text-sage-500 hover:text-sage-600 mb-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {passwords.length === 0 ? <div className="text-center text-gray-500 py-8">
                  No passwords saved yet. Generate and save some passwords!
                </div> : passwords.map(pwd => <div key={pwd.id} className="bg-white/50 backdrop-blur-sm border border-gray-200 rounded-lg p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-medium text-gray-900">{pwd.account}</h3>
                        <input type="password" value={pwd.password} readOnly className="mt-1 bg-transparent border-none text-gray-600 focus:outline-none" />
                      </div>
                      <div className="flex space-x-2">
                        <Button variant="ghost" size="icon" onClick={() => copyToClipboard(pwd.password)} className="hover:bg-gray-200">
                          <Copy className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => deletePassword(pwd.id)} className="hover:bg-gray-200 hover:text-red-500">
